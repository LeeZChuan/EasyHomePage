  ...
  <div class="jn-abilities mt-5 position-relative" id="Abilities">
    ...
    <div class="row justify-content-evenly mt-5 mx-0">
      ...
      <div v-if="loaded" class="jn-right-panel col-md-7 col-12">
        <div
          v-for="i in Math.ceil(markdownFiles.length / 2)"
          :key="i"
          class="row"
        >
          <div v-for="j in 2" :key="j" class="col-md-6 col-12 mb-5">
            <div class="card jn-card jn-abilities-card h-100 jn-animate-card">
              <div
                class="card-body"
                v-if="markdownFiles[(i - 1) * 2 + (j - 1)]"
              >
                <div class="jn-icon">
                  <i
                    class="bi"
                    :class="
                      markdownFiles[(i - 1) * 2 + (j - 1)].attributes.icon
                    "
                  ></i>
                </div>
                <h5 class="card-title">
                  {{ markdownFiles[(i - 1) * 2 + (j - 1)].attributes.Title }}
                </h5>
                <div
                  v-html="markdownFiles[(i - 1) * 2 + (j - 1)].html"
                  class="jn-card-text"
                ></div>
              </div>
            </div>
