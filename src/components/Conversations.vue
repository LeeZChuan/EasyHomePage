    ...
    <div class="row justify-content-evenly mt-5 mx-0">
      <div class="px-5 col-12">
        <div v-if="loaded">
          ...
          >
            <div
              :class="`conversation ${index % 2 === 0 ? 'conversation-right' : 'conversation-left'} col-8 col-md-9`"
              v-html="file.html"
            ></div>
            <div
...
const imageModules = import.meta.glob("/contents/conversations/images/*", {
  eager: true,
});
  ...
  const importPromises = Object.values(markdownContext).map(
    async (loadMarkdown) => {
      ...
      const imagePathKey = Object.keys(imageModules).find((key) =>
        key.includes(attributes.avatar),
      );
